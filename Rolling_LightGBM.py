...
def LGMTrial(n_trials):
    import optuna
    study = optuna.create_study()
    study.optimize(LGMOptuna, n_trials)
    ...
    for i in tqdm.tqdm(YEAR):
        ...
        for k in tqdm.tqdm(range(i-2010)):
            ...
            for v in tqdm.tqdm(range(n)):#down sampling
                ...
                t_test_light = np.array(DF_test['GSTATUS_3m'].tolist())
                # auc_xgb = 0
                # import time
...
# tpr_rolling_light = np.array(df_roc_total_N.mean(axis='columns').tolist())


# rolling_light = pd.DataFrame(
...
# for j in range(10):
#     auc_toal[j]
#     rolling_light[2010+1+j] = auc_toal[j]

# rolling_light
# tpr_rolling_light = np.array(df_roc_total.mean(axis='columns').tolist())
# [round(np.mean(auc),3), '('+ str(round(np.mean(auc)-1.96*stdev(auc),3)) + '-' + str(round(np.mean(auc)+1.96*stdev(auc),3))+')']  

# rolling_light_30days_roc = pd.DataFrame(
#     index=[i for i in range(1001)],
#              )

# rolling_light_30days_roc['fpr'] = [i/1000 for i in range(1001)]
# rolling_light_30days_roc['tpr'] = tpr_rolling_light

# plt.plot(np.array([i/1000 for i in range(1001)]), tpr_rolling_light, color='mediumvioletred', label='LightGBM')
# plt.fill_between(np.array([i/1000 for i in range(1001)]), tpr_rolling_light, 0, color='mediumvioletred', alpha=0.1)
