...
def XGBOptuna(trial):
    ...
    auc = roc_auc_score(t_test_xgb, predicted_xgb)
    # モデルの保存
    # import pickle
...
for i in tqdm.tqdm(range(6)):
    ...
    for j in tqdm.tqdm(range(k)):
        x_train, x_test, t_train, t_test_xgb = train_test_split(x, t, test_size = 1/k)#testsizeを0.09などにすると精度が良くなった。これをここに入れるのが肝。splitの最適化をしにいっているようなもの
        auc_light = 0
        import time
        timeout = time.time() + 60*0.5
        auc_max = []
        best_random = []
        while auc_light <= 0.70:
        ...
        model_xgb = xgb.XGBRegressor(objective = 'binary:logistic', 
                                 ...
                                 alpha = 10, 
                                 n_etimator= 10,
                                 random_state = best_random_j,#これ忘れてた。。。
                                 verbose = 0,
                                 verbosity = 0)
        model_xgb.fit(x_train, t_train)
        predicted_xgb = model_xgb.predict(x_test)
        from sklearn.metrics import roc_curve, roc_auc_score
        fpr, tpr, thresholds = roc_curve(t_test_xgb, predicted_xgb)
        df_roc = pd.DataFrame(
    ...
    df_roc_total_k[i] = df_roc0.mean(axis='columns').tolist()
...
tpr_shuffled_xgb = np.array(df_roc_total_k.mean(axis='columns').tolist())
